...
(function() {
  ...
  function applyUXEnhancements() {
    ...
    // UX-IA-LAYOUT: Reorder IA tab — document accordions appear first (top),
    //   API Key + KB editor + batch action buttons go to bottom.
    //   Approach: wrap rValidarIA so we run AFTER every render.
    (function() {
      function _reorderIATab() {
        var panel = document.querySelector('#mc .ia-panel');
        if (!panel) return;
        ...
        // Classify each direct child into three buckets:
        //   "top"     = progress-bar, contract badge, page header (.phd)
        //   "middle"  = document accordions (contain ia-drop- zones)
        //   "bottom"  = API Key .ia-section, KB .ia-section, batch buttons,
        //               running-progress indicator, unified-report section
        var topEls = [], middleEls = [], bottomEls = [];

        kids.forEach(function(el) {
          if (!el) return;

          // TOP: progress bar or page header
          if (el.id === 'ia-progress-bar' ||
              (el.classList && el.classList.contains('ia-progress-overview')) ||
              (el.classList && el.classList.contains('phd'))) {
            topEls.push(el);
            return;
          }

          // TOP: contract-type badge (small conditional div, no ia-section class)
          var html = el.innerHTML || '';
          if (!el.classList.contains('ia-section') &&
              el.tagName === 'DIV' &&
              html.indexOf('ia-drop-') === -1 &&
              html.indexOf('iaRunAll') === -1 &&
              html.indexOf('iaCruzarDocumentos') === -1 &&
              html.indexOf('ia-api-key') === -1 &&
              html.indexOf('ia-kb-') === -1 &&
              html.indexOf('ia-key-input') === -1 &&
              el.querySelectorAll('textarea').length === 0 &&
              el.querySelectorAll('input[type="password"]').length === 0 &&
              el.querySelectorAll('.ia-doc-accordion').length === 0 &&
              el.querySelectorAll('[id^="ia-drop-"]').length === 0 &&
              el.querySelectorAll('[id^="ia-acc-"]').length === 0) {
            topEls.push(el);
            return;
          }

          // BOTTOM: API Key section (has password input or ia-api-key id)
          if (el.querySelectorAll('input[type="password"]').length > 0 ||
              el.querySelector('#ia-api-key') ||
              html.indexOf('ia-api-key') !== -1 ||
              html.indexOf('ia-key-input') !== -1) {
            bottomEls.push(el);
            return;
          ...
          // BOTTOM: KB editor section (has textarea)
          if (el.querySelectorAll('textarea').length > 0 ||
              html.indexOf('ia-kb-') !== -1) {
            bottomEls.push(el);
            return;
          }

          // BOTTOM: batch action buttons (iaRunAll / iaCruzarDocumentos / iaRelatorio)
          var btns = el.querySelectorAll ? el.querySelectorAll('[onclick]') : [];
          var hasBatch = false;
          for (var i = 0; i < btns.length; i++) {
            var oc = btns[i].getAttribute('onclick') || '';
            if (oc.indexOf('iaRunAll') !== -1 ||
                oc.indexOf('iaCruzarDocumentos') !== -1 ||
                oc.indexOf('iaRelatorio') !== -1) {
              hasBatch = true;
              break;
            }
          }
          if (hasBatch && !el.classList.contains('ia-section')) {
            bottomEls.push(el);
            return;
          }

          // MIDDLE: document accordion sections (contain drop zones or accordion ids)
          if (el.querySelectorAll('[id^="ia-drop-"]').length > 0 ||
              el.querySelectorAll('[id^="ia-acc-"]').length > 0 ||
              el.querySelectorAll('.ia-doc-accordion').length > 0 ||
              html.indexOf('ia-drop-') !== -1) {
            middleEls.push(el);
          ...
          // Default: treat as bottom (running-progress, unified-report, etc.)
          bottomEls.push(el);
        });

        // Require at least some document accordions in middle to proceed
        if (middleEls.length === 0) return;

        // Re-insert in order: top → middle → bottom
        var frag = document.createDocumentFragment();
        topEls.forEach(function(el) { frag.appendChild(el); });
        middleEls.forEach(function(el) { frag.appendChild(el); });
        bottomEls.forEach(function(el) { frag.appendChild(el); });

        // Clear panel and reinsert
        while (panel.firstChild) panel.removeChild(panel.firstChild);
        panel.appendChild(frag);
      }

      // Wrap rValidarIA to run after every IA tab render
      setTimeout(function() {
        var _origRValidarIA = typeof window.rValidarIA === 'function' ? window.rValidarIA : null;
        if (!_origRValidarIA) return;
        window.rValidarIA = function() {
          _origRValidarIA.apply(this, arguments);
          // Run after DOM settles
          requestAnimationFrame(function() { _reorderIATab(); });
        };
        // Run now if already on IA tab
        if (document.querySelector('#mc .ia-panel')) _reorderIATab();
      }, 500);
    })();
  }
  ...
  (function applySecurityPatches() {
    ...
    // SEC-NEW-3: Session expiry — clear API key after 4h inactivity
    (function() {
      var INACTIVE_MS = 4 * 60 * 60 * 1000; // 4 hours
      var _lastActivity = Date.now();
      var _EVENTS = ['mousemove','keydown','click','touchstart'];
      _EVENTS.forEach(function(ev) {
        window.addEventListener(ev, function() { _lastActivity = Date.now(); }, {passive: true});
      });
      setInterval(function() {
        if (Date.now() - _lastActivity > INACTIVE_MS) {
          try {
            sessionStorage.removeItem('_lft_k');
            console.info('[LOFT-SEC] Chave de API removida por inatividade (4h).');
          } catch(ex) {}
        }
      }, 5 * 60 * 1000); // check every 5min
    })();

    // SEC-NEW-4: Extended blob URL expiry (5s) and API key leak detection
    (function() {
      // Wrap URL.createObjectURL to track and revoke blob URLs after 5s (instead of 2s)
    ...
    // SEC-NEW-5: API key leak detection — warn if key appears in visible DOM text
    setTimeout(function() {
      try {
        var key = sessionStorage.getItem('_lft_k');
        if (!key) return;
        var decoded = typeof window._xorDec === 'function' ? '' : ''; // key is encoded
        // Just check if any text node contains something that looks like an API key (AIzaSy...)
        var body = document.body.textContent || '';
        var keyMatch = body.match(/AIzaSy[A-Za-z0-9_-]{33}/);
        if (keyMatch) {
          console.warn('[LOFT-SEC] ⚠ API key parece estar visível no DOM! Recarregue a página e não compartilhe screenshots.');
          loftToast && loftToast('⚠ Chave API pode estar exposta no DOM. Verifique.', 'error', 8000);
        }
      } catch(ex) {}
    }, 3000);
  })();
