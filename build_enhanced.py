...
(function() {
  ...
  function applyUXEnhancements() {
    ...
    //   everything else (API Key, KB editor, batch buttons) goes to bottom.
    ...
    (function() {
      function _reorderIATab() {
        ...
        //   "top"    = #ia-progress-bar  |  .phd header
        //   "mid"    = document accordions: id starts with "ia-acc-"
        //   "bottom" = everything else (API Key, KB, batch buttons, etc.)
        var topEls = [], midEls = [], botEls = [];

        kids.forEach(function(el) {
          if (!el) return;
          // TOP: progress bar
          if (el.id === 'ia-progress-bar') { topEls.push(el); return; }
          // TOP: page header
          if (el.classList && el.classList.contains('phd')) { topEls.push(el); return; }
          // MID: individual document accordions (id="ia-acc-contrato", etc.)
          if (el.id && el.id.indexOf('ia-acc-') === 0) { midEls.push(el); return; }
          // BOTTOM: everything else (API Key, KB, batch buttons, running-progress, reports)
          botEls.push(el);
        });

        // Only reorder if there are document accordions to show at top
        if (midEls.length === 0) return;

        // Re-insert: top → accordions → bottom controls
        var frag = document.createDocumentFragment();
        topEls.forEach(function(el) { frag.appendChild(el); });
        midEls.forEach(function(el) { frag.appendChild(el); });
        botEls.forEach(function(el) { frag.appendChild(el); });
